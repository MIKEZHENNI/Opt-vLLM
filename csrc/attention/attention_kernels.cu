...
//block_sum 是一个用于并行规约的函数，计算一个线程块内的全局和
...
inline __device__ float block_sum(float* red_smem, float sum) {
  ...
  // #pragma unroll 用于指示编译器进行循环展开优化，旨在减少循环控制开销来提高性能，在此函数中，其对两个归约循环进行展开
#pragma unroll
...
template<
  ...
  bool IS_FP8_E5M2_KV_CACHE,
  int NUM_THREADS = 128>
  //这两个启动器负责配置CUDA内核的启动参数（如网格大小、块大小、共享内存大小等），并调用相应的内核
