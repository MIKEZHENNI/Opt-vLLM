...
#endif
//块交换：在不同设备（CPU/GPU）之间或同一GPU上的不同内存区域之间复制KV缓存。
void swap_blocks(
  torch::Tensor& src,
  torch::Tensor& dst,
...
//块拷贝（copy_blocks） ：根据映射关系将KV缓存从源块复制到目标块
void copy_blocks(
...
__global__ void reshape_and_cache_kernel(
  ...
  //在KV缓存存储阶段（reshape_and_cache_kernel），跳过小部分冗余块来降低带宽压力，提高吞吐量
 //根据块索引（slot index）的数值特性（如模运算结果）动态跳过部分块的存储
   if (slot_idx >0.5) {  // 或自定义条件（如 slot_idx > THRESHOLD）
    return ;
  }
...
#define CALL_RESHAPE_AND_CACHE(KV_T, CACHE_T, IS_FP8_E5M2_KV_CACHE)                                \
  vllm::reshape_and_cache_kernel<KV_T, CACHE_T, IS_FP8_E5M2_KV_CACHE><<<grid, block, 0, stream>>>( \
    ...
    block_size,                                                                                    \
    x);
//KV缓存重塑与存储（reshape_and_cache） ：将输入的Key和Value张量重新组织并存储到KV缓存中
...
__global__ void gather_cached_kv_kernel(
  ...
  const cache_t* __restrict__ key_cache,  // [num_blocks, num_heads, head_size/x, block_size, x]
  ...
  const int x) {
    ...
    for (int i = threadIdx.x; i < dim; i += blockDim.x) {
      ...
      if constexpr (is_fp8_e5m2_kv_cache) {
#ifdef ENABLE_FP8_E5M2
        key[tgt_key_idx] = fp8_e5m2_unscaled::vec_conversion<scalar_t, cache_t>(VLLM_LDG(&key_cache[src_key_idx]));
        value[tgt_value_idx] = fp8_e5m2_unscaled::vec_conversion<scalar_t, cache_t>(VLLM_LDG(&value_cache[src_value_idx]));
#else
        assert(false);
#endif
      } else {
...
void gather_cached_kv(
  ...
  const std::string& kv_cache_dtype) {  // 新增参数适配 FP8
  int num_tokens = key.size(0);
  int num_heads = key.size(1);
  int head_size = key.size(2);
  int block_size = key_cache.size(3);
  ...
  if (kv_cache_dtype == "auto") {
    VLLM_DISPATCH_FLOATING_AND_BYTE_TYPES(
      ...
      [&] {
        vllm::gather_cached_kv_kernel<scalar_t, scalar_t, false><<<grid, block, 0, stream>>>(
    ...
    VLLM_DISPATCH_FLOATING_TYPES(
      key.scalar_type(),
      "gather_cached_kv_kernel_fp8",
      [&] {
        vllm::gather_cached_kv_kernel<scalar_t, uint8_t, true><<<grid, block, 0, stream>>>(
          key.data_ptr<scalar_t>(),
          value.data_ptr<scalar_t>(),
